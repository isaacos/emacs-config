...
;;(global-set-key key-translation-map (kbd "C-;") 'event-apply-meta-modifier)
...
;; sets space to control in normal mode
;;(defun my-set-space-leader-hook ()
;;  (define-key key-translation-map (kbd "CTRL") nil)
;;  (define-key key-translation-map (kbd "SPC") 'event-apply-control-modifier)
;;  (defun my-set-control-leader-hook ()
;;    (define-key key-translation-map (kbd "SPC") nil))
;;    (define-key key-translation-map (kbd "CTRL") 'event-apply-control-modifier))
;;
;;(add-hook 'evil-normal-state-entry-hook 'my-set-space-leader-hook)
;;(add-hook 'evil-insert-state-entry-hook 'my-set-control-leader-hook)
